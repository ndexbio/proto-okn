 ...
 */
export function buildUri(identifier, namespaces) {
 ...
 */
export function buildStatementUri(baseUri, edgeId, relationshipIndex) {
    return `${baseUri}statement_${edgeId}_${relationshipIndex}`;
}
/**
 * Build RO predicate URI
 */
export function buildRoUri(roId) {
 ...
 * Build GO term URI
 */
export function buildGoUri(goId) {
 ...
 */
export function buildSioUri(sioId) {
 ...
 */
export function nodeTypeToSioUri(nodeType) {
    const typeMap = {
 ...
 */
export function isValidPrefixedUri(identifier, namespaces) {
    const colonIndex = identifier.indexOf(':');
    if (colonIndex === -1)
        return false;
    const prefix = identifier.substring(0, colonIndex);
 ...
 */
export function formatUriForTurtle(uri, namespaces) {
