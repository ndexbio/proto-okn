 ...
 */
export function writeToTurtle(rdfOutput) {
 ...
 */
function writeNodeDeclaration(writer, node, namespaces) {
    const subjectUri = resolveUri(node.uri, namespaces);
    // Type triple
    writer.addQuad(namedNode(subjectUri), namedNode(RDF_TYPE), namedNode(node.type));
    // Label triple
    writer.addQuad(namedNode(subjectUri), namedNode(RDFS_LABEL), literal(node.label));
    // Alias triples (owl:sameAs)
    if (node.aliases) {
        for (const alias of node.aliases) {
            const aliasUri = resolveUri(alias, namespaces);
            writer.addQuad(namedNode(subjectUri), namedNode(OWL_SAMEAS), namedNode(aliasUri));
        }
    }
}
/**
 ...
 */
function writeDirectTriple(writer, triple, namespaces) {
    const subjectUri = resolveUri(triple.subject, namespaces);
    const objectUri = resolveUri(triple.object, namespaces);
    writer.addQuad(namedNode(subjectUri), namedNode(triple.predicate), namedNode(objectUri));
}
/**
 * Write reified statement with metadata
 */
function writeReifiedStatement(writer, statement, namespaces) {
    ...
    // Evidence count
    writer.addQuad(statementNode, namedNode(OKN_EVIDENCE_COUNT), literal(statement.evidenceCount.toString(), namedNode('http://www.w3.org/2001/XMLSchema#integer')));
    // Evidence URL
    writer.addQuad(statementNode, namedNode(OKN_EVIDENCE_URL), namedNode(statement.evidenceUrl));
    // Process type (if present)
    if (statement.processType) {
        writer.addQuad(statementNode, namedNode(OKN_PROCESS_TYPE), namedNode(statement.processType));
    }
}
/**
 * Resolve prefixed URI to full URI
 */
function resolveUri(uri, namespaces) {
    // Already a full URI
 ...
 */
export function writeToTurtleSync(rdfOutput) {
    const chunks = [];
...
//# sourceMappingURL=turtle-writer.js.map